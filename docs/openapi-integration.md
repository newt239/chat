...
- OpenAPI スキーマ: `openapi/openapi.yaml`
...
- `openapi/openapi.yaml` を中心に `paths.yaml` / `components/schemas.yaml` に分割定義
...
```bash
dredd openapi/openapi.yaml http://localhost:8080
```
...
1. Spectral による OpenAPI Lint
   ...
   ```bash
   npx @stoplight/spectral lint openapi/openapi.yaml
   ```
...
jobs:
  ...
  contract-test:
    ...
    steps:
      ...
      - run: schemathesis run --checks all --base-url http://localhost:8080 openapi/openapi.yaml
